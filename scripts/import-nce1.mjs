// scripts/import-nce1.mjs
// 新概念英语第一册：从 JSON 导入数据库
// 源：docs/new_concept_english/1_format/*.json（由 convert-nce1.mjs 生成）
// 用法：先启动 dev server (pnpm dev)，再运行：
//   node scripts/import-nce1.mjs              # 拒绝任何 aligned:false 文件
//   node scripts/import-nce1.mjs --skip-unaligned  # 跳过未对齐文件，仅导入已对齐的
// 幂等：教材按 stage+name+publisher 复用；单词 upsert 去重；导入前清空该教材旧句子
...
const SRC_DIR = new URL('../docs/new_concept_english/1_format/', import.meta.url).pathname

const textbook = { stage: '其他', name: '新概念英语第一册', publisher: '外研社' }

// 手动加载 .env（取 DB 连接信息，用于清理旧句子）
