...
function getTeachingFinal(key) {
  ...
  if (dictFinal === 'v')
    return { initial, final: 'ü' }
  if (dictFinal === 've')
    return { initial, final: 'üe' }
  if (dictFinal === 'ue')
    return { initial, final: 'üe' }
  if (dictFinal === 'un' && initial && ['j', 'q', 'x', 'y'].includes(initial))
    return { initial, final: 'ün' }
  if (dictFinal === 'uan' && initial && ['j', 'q', 'x', 'y'].includes(initial))
    return { initial, final: 'üan' }
...
for (const k of keys) {
  for (const q of oldDict[k]) origWords.add(q.words)
}
