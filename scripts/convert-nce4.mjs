// scripts/convert-nce4.mjs
// 新概念英语第四册：逐文件 GBK TXT → JSON 转换器
// 源：docs/new_concept_english/4/*.TXT（GBK）
// 产物：docs/new_concept_english/4_format/{lesson}.json
// 基于 NCE3，新增：作者署名排除 + 生词锚点单复数兼容
import { execSync } from 'node:child_process'
import { mkdirSync, readdirSync, rmSync, writeFileSync } from 'node:fs'
import { join } from 'node:path'

const SRC_DIR = new URL('../docs/new_concept_english/4/', import.meta.url).pathname
const OUT_DIR = new URL('../docs/new_concept_english/4_format/', import.meta.url).pathname
...
function splitSections(text) {
  ...
  // NCE4: 部分课 "New words and expression"（单数）或无中文后缀
  const wordIdx = lines.findIndex(l => /New words and expressions?|生词和短语/i.test(l))
  const transIdx = lines.findIndex((l, i) => i > wordIdx && l.includes('参考译文'))

  // 课文区间 = listenIdx 到 wordIdx 之间
  // 逐步跳过：EN 指令 → ZH 指令 → 空行 → 可选理解问题(以?结尾) → 空行
  ...
  while (start < textLines.length && textLines[start].trim() === '')
    start++ // 空行
  let passageLines = textLines.slice(start)

  // 去除尾部空行
  while (passageLines.length > 0 && passageLines[passageLines.length - 1].trim() === '')
    passageLines = passageLines.slice(0, -1)
  // NCE4: 排除作者署名（最后一行不以句末符结尾）
  if (passageLines.length > 0) {
    const last = passageLines[passageLines.length - 1].trim()
    if (!/[.!?]["')\]]?$/.test(last))
      passageLines = passageLines.slice(0, -1)
  }
 ...
 * NCE4 格式（两行制，同 NCE1/NCE3），部分词带 (title) 注释
 */
...
function parseTitle(titleLines, lesson) {
  ...
  const en = text.find(l => !/Lesson\s|Unit\s/i.test(l) && /[a-z]/i.test(l)) ?? ''
