// scripts/import-nce4.mjs
// 新概念英语第四册：从 JSON 导入数据库
...
const SRC_DIR = new URL('../docs/new_concept_english/4_format/', import.meta.url).pathname
const textbook = { stage: '其他', name: '新概念英语第四册', publisher: '外研社' }
...
for (const { path } of files)
  lessons.push(JSON.parse(readFileSync(path, 'utf8')))
...
console.log(`  ${sentenceItems.length - nullTrans} 句有译文、${nullTrans} 句待补\n`)
...
console.log(`  ✓ ${wordResult.total} 条：新建 ${wordResult.created}，更新 ${wordResult.updated}`)
...
console.log(`  ✓ ${sentenceItems.length} 条：新建 ${totalCreated}，跳过 ${totalSkipped}`)
